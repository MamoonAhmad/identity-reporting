...
export const SidePanel = () => {
    ...
    const [state, setState] = useGeneralState<SidePanelState>({ComponentToShow: NullComponent, zIndex: undefined})

    const { state: { SidePanelComponents }, setState: setStoreState, getNextZIndex } = useContext(StoreContext)

    useEffect(() => {
        if(SidePanelComponents?.length) {
            setState({ComponentToShow: SidePanelComponents[SidePanelComponents?.length -1]})
        } else {
    ...
    return (
        ...
        <div className="flex flex-col transition-transform ease-in-out delay-75 overflow-scroll  bg-blue-100 shadow-lg fixed top-0 right-0" style={{
