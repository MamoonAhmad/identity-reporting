...
export type ObjectValidatorConfigJSON = Omit<ValidatorConfigJSON, 'targetValue'> & {
...
export class ObjectValidator extends Validator{
    ...
    json (): ObjectValidatorConfigJSON {
        ...
        Object?.keys(this.config.targetValue).forEach(k => newTarget[k] = this.config.targetValue![k].json())
